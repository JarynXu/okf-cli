...
impl ProcessLibraryProvider {
    ...
    fn invoke(&self, request: &ProviderRequest) -> LibraryResult<ProviderResponse> {
        ...
        let stdout = child
            .stdout
            .take()
            .ok_or_else(|| LibraryError::Provider("process stdout was not piped".into()))?;
        let stderr = child
            .stderr
            .take()
            .ok_or_else(|| LibraryError::Provider("process stderr was not piped".into()))?;
        let stdout_reader = thread::spawn(move || read_all(stdout));
        let stderr_reader = thread::spawn(move || read_all(stderr));
        let bytes = serde_json::to_vec(request).map_err(|error| {
            LibraryError::Provider(format!("failed to encode provider request: {error}"))
        })?;
        ...
        let stdout = stdout_reader
            .join()
            .map_err(|_| LibraryError::Provider("provider stdout reader panicked".into()))??;
        let stderr = stderr_reader
            .join()
            .map_err(|_| LibraryError::Provider("provider stderr reader panicked".into()))??;
        if !status.success() {
...
fn io_error(error: impl std::fmt::Display) -> LibraryError {
    LibraryError::Provider(format!("process provider I/O error: {error}"))
}
